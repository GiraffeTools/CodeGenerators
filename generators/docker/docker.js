...
async function nodeCode(nodes) {
  ...
  const languagesInEditor = dockerCodes.map(
      (code) => code.argument &&
        code.argument.name && code.argument.name.split(',')
  );
  ...
  const toolboxes = [
    ...new Set(
        languagesInEditor.filter((l) => l).reduce((acc, val) =>
          acc.concat(val), [])
    ),
  ]
...
export async function writeFiles(nodes, links) {
  const dockerFilename = 'GIRAFFE/code/Dockerfile';
  const dockerComposeFilename = 'GIRAFFE/code/docker-compose.yml';
  const emptyFileTemp = 'GIRAFFE/code/temp/.empty';
  const emptyFileOutput = 'GIRAFFE/code/output/.empty';

  return {
    [dockerFilename]: await writeCode(nodes),
    [dockerComposeFilename]: await (await fetch(
        '/static/assets/misc/docker-compose.yml'
    )).text(),
    [emptyFileTemp]: await (await fetch(
        '/static/assets/misc/empty.txt'
    )).text(),
    [emptyFileOutput]: await (await fetch(
        '/static/assets/misc/empty.txt'
    )).text(),
  };
}
