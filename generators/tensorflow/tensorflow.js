...
const writeNodes = (nodes, links) => {
  ...
  const nodeSequence = [firstNode.id];
  let child = getChild(firstNode.id);
  while (child && !nodeSequence.includes(child)) {
    nodeSequence.push(child);
    child = getChild(child);
  }

  const nodeList = nodeSequence
...
const writePostamble = (nodes, links) => {
  ...
  const createModel =
    ...
    _model = tf.keras.models.Model(
      inputs  = [${firstNode.name}],
      outputs = [${nodes.find((node) =>
    node.id == nodeSequence.slice(-1)).name}]
    )
`;
  const compileModel = `
    _model.compile(
...
export function writeFiles(nodes, links) {
  const pythonFile = 'GIRAFFE/code/neural_net.py';
  const initFile = 'GIRAFFE/code/__init__.py';
